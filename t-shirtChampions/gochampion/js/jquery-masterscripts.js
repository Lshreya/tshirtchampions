...
$(function() {
		// set opacity to nill on page load
		$("ul#button_contact span").css("opacity","0");
		// on mouse over
		$("ul#button_contact span").hover(function () {
			// animate opacity to full
			$(this).stop().animate({
				opacity: 1
			}, "medium");
...
$(document).ready(function() {
	...
	var href = $('#navload li a').each(function(){
		var href = $(this).attr('href');
		if(hash==href.substr(0,href.length-5)){
			var toLoad = hash+'.html #contentload2';
			$('#contentload2').load(toLoad)
		}											
	});

	$('#navload li a').click(function(){
								  
		var toLoad = $(this).attr('href')+' #contentload2';
		$('#contentload2').hide('slow',loadContent);
		$('#load').remove();
		$('#wrapperload').append('<span id="load">LOADING...</span>');
		$('#load').fadeIn('normal');
		window.location.hash = $(this).attr('href').substr(0,$(this).attr('href').length-5);
		function loadContent() {
			$('#contentload2').load(toLoad,'',showNewContent())
		}
		function showNewContent() {
			$('#contentload2').show('normal',hideLoader());
		}
		function hideLoader() {
			$('#load').fadeOut('normal');
		}
