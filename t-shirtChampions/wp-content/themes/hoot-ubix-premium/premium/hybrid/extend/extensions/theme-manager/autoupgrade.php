...
// @note
// edd API Calls:
// * when print theme manager content (to check license) - once a day using transient
// * when user clicks activate/deactivate
// * when user saves/updates license key ( via activate_license() )
// {updater.php}
...
class HybridExtend_Theme_Manager_Autoupgrade extends HybridExtend_Theme_Manager {
	...
	public function __construct() {
		...
		if ( isset( $_GET['force_check_version'] ) && $_GET['force_check_version'] == 'true' )
		...
		add_action( 'update_option_' . HYBRIDEXTEND_THEMESLUG . '_license_key', array( $this, 'activate_license' ), 10, 2 );
	...
	function print_autoupgrade() {
		...
		$status = get_option( HYBRIDEXTEND_THEMESLUG . '_license_key_status', false );
		...
		if ( ! $license ) {
			...
			// delete_transient( HYBRIDEXTEND_THEMESLUG . '_license_message' );
			if ( ! get_transient( HYBRIDEXTEND_THEMESLUG . '_license_message' ) ) {
				set_transient( HYBRIDEXTEND_THEMESLUG . '_license_message', $this->check_license(), ( 60 * 60 * 24 * 15 ) ); // since we have lifetime licenses, lets set this to 15 instead of 1 day
			}
		...
		<div id="hybridextend-theme-mgr-autoupgrade" class="hybridextend-theme-mgr">
			...
			<form action="options.php" method="post">
				...
				<?php if ( !empty( $license ) ) : ?>
					...
					<?php if ( 'valid' == $status ) { ?>
						<input type="submit" class="button button-secondary hybridextend-deactivate-license" name="<?php echo HYBRIDEXTEND_THEMESLUG; ?>_license_deactivate" value="<?php _e( 'Deactivate License for this Site', 'hybrid-core' ); ?>"/>
					<?php } else { ?>
						<input type="submit" class="button button-secondary hybridextend-activate-license" name="<?php echo HYBRIDEXTEND_THEMESLUG; ?>_license_activate" value="<?php _e( 'Activate License for this Site', 'hybrid-core' ); ?>"/>
					<?php } ?>
					<p class="description"><?php echo $message; ?></p>
				<?php endif; ?>
				<?php submit_button( __( 'Save License Key', 'hybrid-core' ) ); ?>
			</form>
	...
	function check_license() {
		...
		// Only activate(valid/invalid) deactivate(activated/deactivated) should set status
		// as check_license will return status of license (valid/invalid)e, not whether current site
		// is active or not. Eg: License is valid, but site is deactivated, status returned is "valid"
		// 
		// @todo ISSUE: What if user deactivates from wphoot site? License here will keep
		// showing active
		// 
		// if ( $license_data && isset( $license_data->license ) ) {
		// 	update_option( HYBRIDEXTEND_THEMESLUG . '_license_key_status', $license_data->license );
	...
	function get_api_response( $api_params ) {
		...
		// Call the custom API.
		$response = wp_remote_post( $this->args['remote_api_url'], array( 'timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
	...
	function updater() {
		...
		if ( get_option( HYBRIDEXTEND_THEMESLUG . '_license_key_status', false) != 'valid' ) {
			return;
		}
