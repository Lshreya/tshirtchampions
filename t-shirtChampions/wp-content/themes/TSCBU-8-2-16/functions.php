...
add_filter('frm_pre_create_entry', 'set_hidden_fields');
function set_hidden_fields($values){
  global $post,$state_url_list;
  if($values['form_id'] == 6){
    $values['item_meta'][105] = $values['item_meta'][88] . ", " . $state_url_list[strtolower($values['item_meta'][84])]; //City, State
    $values['item_meta'][106] = $values['item_meta'][83] . ", " . $values['item_meta'][85]; //First Name Last Initial
  }
  return $values;
}
